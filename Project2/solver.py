...
class Node:
    ...
    def validMoveAlt(self,fromCol,toCol):
        ...
        for i in range(0, self.ntubes):
            for j in range(0, self.ntubes):
                ...
                if self.validMove(i, j) and i != y[1]:
                    newstate = Node(self, copy.deepcopy(self.getMatrix()), copy.deepcopy(
                        self.getArrCompleted()), self.n, self.m, self.ntubes, (i, j), self.getDepth()+1, 0)
                    newstate.moveBall(i, j)
                    newstate.evaluateState()
                    childs.append(newstate)
        return childs
    
    def generateChildsAlt(self):
        childs=[]
        for i in range(0,self.ntubes):
            for j in range(0,self.ntubes):
                y = self.getLastMove()
                if self.validMoveAlt(i,j) and i!=y[1]:
...
class Graph:
    ...
    def bfsStates(self,node):
        ...
        while states:
            state=states.pop(0)     
            children = state.generateChildsAlt()
